...
<div class="admissionpage  py-xl-5 py-lg-5 py-md-4 py-sm-4 py-4" id="admissionpage">
    <div class="container">
        <div class="row bg-white">
            ...
            <div class="col-12">
                <form class="row g-3 showInterestform" name="mba_brochure_form" id="mba_brochure_form" method="POST" action="{{ url('downloadBrochure') }}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <p class="flash-message-mba" style="color:green"></p>
                </form>
            </div>
        </div>
    </div>
...
@push ('after-scripts')
    ...
    $('#mba_brochure_form').on('submit', function(e) {
        e.preventDefault();
        var email = $("#email").val();
        var name = $("#name").val();
    ...
    $.ajax({
     ...
     success: function()
                {
                    $('.flash-message-mba').html("Brochure Submit Successfully");
                    $('.flash-message-mba').fadeOut(5000);
                    setTimeout(() => {
                        var element = document.getElementById("mba_brochure_form");
                    
                    element.reset();
                    }, 5500);
                },
      error:function(){
        $('.flash-message-mba').html("Brochure Not Submit Successfully");
                    $('.flash-message-mba').fadeOut(5000);
                     setTimeout(() => {
                        var element = document.getElementById("mba_brochure_form");
                        grecaptcha.reset();
