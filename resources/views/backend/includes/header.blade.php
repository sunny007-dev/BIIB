...
<header class="header header-sticky mb-4">
    <div class="container-fluid">
        <button class="header-toggler px-md-0 me-md-3" type="button" onclick="coreui.Sidebar.getInstance(document.querySelector('#sidebar')).toggle()">
            ...
            <img src="{{asset('img/toggler.png')}}" alt="..." style="height: 28px;">
        </button>
        ...
        <ul class="header-nav ms-auto">
            <li class="nav-item dropdown">
                <a class="nav-link notification" data-coreui-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                    <svg class="icon icon-lg">
                        <use xlink:href="/fonts/free.svg#cil-bell"></use>
                    </svg>@if($notifications_count)<span class="badge badge-pill bg-danger">{{$notifications_count}}</span>@endif
                </a>
        ...
        <ul class="header-nav ms-3">
            ...
            <li class="nav-item dropdown">
                <a class="nav-link activeUser py-0" data-coreui-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false">
                    <div class="avatar">
                        <img class="avatar-img" src="{{ asset(auth()->user()->avatar) }}" alt="{{ asset(auth()->user()->name) }}">
                    </div>
                    <p class="m-0 ms-2">{{ auth()->user()->name }} <i class="fa fa-angle-down" aria-hidden="true"></i>
</p>
                </a>
                <div class="dropdown-menu dropdown-menu-end pt-0">
                    <div class="dropdown-header bg-light py-2">
                    ...
                    <a class="dropdown-item" href="{{route('backend.users.profile', Auth::user()->id)}}">
                        <i class="cil-laptop me-2"></i>&nbsp; @lang('View Profile')
                    </a>
