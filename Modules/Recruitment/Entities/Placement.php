...
use Modules\PageMenu\Entities\Presenters\PagePresenter;
...
class Placement extends BaseModel
{
    ...
    {
        return $this->belongsTo('Modules\Recruitment\Entities\Recruiter');
    }



    public function setRecruiterIdAttribute($value)
    {
        $this->attributes['recruiter_id'] = $value;

        try {
            $category = Recruiter::findOrFail($value);
            $this->attributes['recruiter_name'] = $category->name;
        } catch (\Exception $e) {
            $this->attributes['recruiter_name'] = null;
        }
    }
