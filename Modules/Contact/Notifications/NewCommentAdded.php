...
class NewCommentAdded extends Notification implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct(Contact $comment)
    {
    ...
    public function toSlack($notifiable)
    {
        ...
        return (new SlackMessage())
                ...
                ->content('New Contact: '.$comment->name.' | From:'.$comment->user_name)
                ->attachment(function ($attachment) use ($comment) {
                    $attachment->title('Contact '.$comment->id, route('backend.comments.show', $comment->id))
                    ->fields([
    ...
    public function toDatabase($notifiable)
    {
        ...
        $text = 'New Contact | <strong>'.$comment->name.'</strong> on <strong>'.$comment->post_name.'</strong>  by <strong>'.$comment->user_name.'</strong>';

        $url_backend = route('backend.comments.show', $comment->id);

        return [
            'title'         => 'New Contact Form Filled!',
            'module'        => 'Contact',
